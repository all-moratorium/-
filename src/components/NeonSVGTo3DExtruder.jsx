...
const NeonSVGTo3DExtruder = forwardRef(({ neonSvgData, backgroundColor = '#242424', modelData, onNavigateToInfo, isGuideEffectStopped, onGuideEffectStop }, ref) => {
  ...
  const loadSVGFile = useCallback((file) => {
    ...
    reader.onload = (e) => {
      ...
      const smoothUpdateProgress = (progress, message) => {
        ...
        const animate = () => {
          ...
          window.dispatchEvent(new CustomEvent('3DProgressUpdate', {
            detail: { progress: Math.round(currentProgress), message }
          }));
          ...
          if (Math.abs(targetProgress - currentProgress) > 0.5) {
            ...
            window.dispatchEvent(new CustomEvent('3DProgressUpdate', {
              detail: { progress: targetProgress, message }
            }));
          }
        };
      ...
      const updateProgress = smoothUpdateProgress;
        ...
        if (child.geometry.type === 'TubeGeometry') {
          ...
          if (path && path.points && path.points.length > 1) {
            ...
            const newGeometry = new THREE.TubeGeometry(
              simplifiedCurve,
              Math.max(120, simplifiedPoints.length * 1), // tubularSegments（さらに増加）
              originalRadius,
              10, // radialSegments（10→12に増加
              false
            );
    ...
    <div className="neon-container">
      ...
      <div className={`controls-panel ${sidebarVisible ? '' : 'collapsed'}`}>
        ...
        <div className="neon3d-details-info-container">
          ...
          <div className="neon3d-dimension-item">
            <span className="neon3d-dimension-label">OFF時の<br />チューブカラー</span>
            <span className="neon3d-dimension-value neon3d-small-text">{calculatedModelData?.isGenerated === true ? (offTubeColor === 'white' ? 'ホワイト' : '発光色') : 'N/A'}</span>
          </div>
          <div className="neon3d-dimension-item">
        ...
        <div className="neon3d-room-model-selector">
          <div className="neon3d-dimension-item">
            ...
            >
              ...
              <option value="neonbrick6glb-v1">ネオンレンガ</option>
